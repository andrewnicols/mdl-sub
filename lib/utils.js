...
/**
* Walk the directory tree to locate the mdl.json file.
...
var getProjectData = function(dir) {
    ...
    while (dirs.length && !projectData) {
        if (dirCount++ > 5000) {
            Y.log('Scanned ' + dirCount + ' directories looking for a .mdl.json file, something is probably wrong here..', 'error', Y.packageInfo.name);
            process.exit(1);
        }
        ...
        dirs.forEach(function(dir) {
            ...
            try {
                ...
                names.forEach(function(name) {
                    ...
                    if (Y.Files.isDirectory(p)) {
                        ...
                        if (name === 'node_modules') {
                            Y.log('Skipping node_modules directory while scanning for .mdl.json', 'warn', Y.packageInfo.name);
                            return;
                        }
                        childDirs.push(p);
                    }
                ...
                Y.log('Accessing dir (' + dir + ') threw an error', 'warn', Y.packageInfo.name);
            }
        });
        // iterate over new set of folders
        dirs = childDirs;
    }
    if ((packageData && projectData) || (packageData && packageData.mdl)) {
...
var mergeData = function(pack, project) {
    ...
    if (pack.mdl) {
        Object.keys(pack.mdl).forEach(function(key) {
            if (!project[key]) {
                project[key] = pack.mdl[key];
            }
        });
    }
